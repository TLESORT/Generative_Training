...
resume = 'checkpoint_cvae_conv_fashionmnist.pth.tar'
...
train_loader = torch.utils.data.DataLoader(datasets.FashionMNIST('fashion_data', train=True, download=True, transform=transforms.ToTensor()),
               batch_size=batch_size, shuffle=True, num_workers=8, pin_memory=True)
test_loader = torch.utils.data.DataLoader(datasets.FashionMNIST('fashion_data', train=False, download=True, transform=transforms.ToTensor()),
              batch_size=batch_size, shuffle=True, num_workers=8, pin_memory=True)
...
if args.train_vae:
    ...
    print_samples(img[0:144], 1, 144, 'samples_conv_fashion_mnist.png')
...
if args.train_classifier:
    ...
    for epoch in range(1, epochs + 1):
        ...
        test_acc.append(acc)
    np.savetxt('data_classif_fashion_mnist.txt', np.transpose([train_loss, train_acc, test_loss, test_acc]))
